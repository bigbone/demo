...
(function($, window, Hammer) {
    ...
    $.fn.ImageEditor = function(options) {
        ...
        ImageEditor.prototype = {
            ...
            _click: function() {
                new Hammer(this.$el[0]).on('tap', function(e) {
                    var target = $(e.target);
                    //console.log(target);
                    if (target.attr('img-item')) {
                        var index = target.attr('[img-item]');
                        if (this.images[index - 1] && this.images[index - 1]['clickToSelect']) {
                            this.selectImage(index - 1);
                        }
                        //console.log(index);
                    } else if (target.parents('[img-item]')) {
                        var index = target.parents('[img-item]').attr('img-item');
                        if (this.images[index - 1] && this.images[index - 1]['clickToSelect']) {
                            this.selectImage(index - 1);
                        }
                        //console.log(index);
            ...
            _placeImage: function(image) {
                ...
                $imgWrapper.attr('img-item', index);
                // set click to select
                ...
                new Hammer($imgWrapper[0]).on('tap', function() {
                    /*document.title = image.img.src.replace('http://demo.weibo.com/canvas/imgeditor/', '');
